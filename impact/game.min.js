...
ig.baked=true;ig.module('game.entities.qubber').requires('impact.entity').defines(function(){EntityQubber=ig.Entity.extend({type:ig.Entity.TYPE.A,checkAgainst:ig.Entity.TYPE.B,collides:ig.Entity.COLLIDES.ACTIVE,animSheet:new ig.AnimationSheet('media/player.png',16,28),size:{x:16,y:28},flip:true,accelGround:200,accelAir:310,jump:360,friction:{x:500,y:500},maxVel:{x:160,y:160},money:0,init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('idle',1,[1]);this.addAnim('run',0.20,[1,0]);this.addAnim('jump',1,[2]);this.addAnim('fall',0.4,[2]);},update:function(){var accel=this.accelAir;if(ig.input.state('left')){this.accel.x=-accel;this.flip=false;}else if(ig.input.state('right')){this.accel.x=accel;this.flip=true;}else
...
this.levels=this.levels.concat(newLevels);return true;}else{return false;}},});});
...
// lib/game/main.js
ig.baked=true;ig.module('game.main').requires('impact.game','impact.debug.debug','impact.font','game.levels.qubmap','game.levels.mainMenu','game.entities.qubber','game.entities.house','game.controllers.sceneController','game.controllers.playerController').defines(function(){MyGame=ig.Game.extend({font:new ig.Font('media/04b03.fontblack.png'),bigfont:new ig.Font('media/04b03.fontblackx3.png'),biggerfont:new ig.Font('media/04b03.fontblackx5.png'),name:"Quber",player:null,STATE:{MAINMENU:0,PLAYMODE:1,APPMODE:2,MINIGAME:3},init:function(){ig.input.bind(ig.KEY.LEFT_ARROW,'left');ig.input.bind(ig.KEY.RIGHT_ARROW,'right');ig.input.bind(ig.KEY.UP_ARROW,'up');ig.input.bind(ig.KEY.DOWN_ARROW,'down');ig.input.bind(ig.KEY.A,'left');ig.input.bind(ig.KEY.D,'right');ig.input.bind(ig.KEY.W,'up');ig.input.bind(ig.KEY.S,'down');ig.input.bind(ig.KEY.J,'pause');ig.input.bind(ig.KEY.Q,'app');ig.input.bind(ig.KEY.ENTER,'enter');this.currentState=this.STATE.MAINMENU;this.drawCoordinates={main:{x:ig.system.width/2,y:ig.system.height/2-50},score:{x:5,y:5},profile:{x:5,y:ig.system.height-10}}
this.sceneController=new ig.sceneController(this,[LevelMainMenu,LevelQubmap])
this.playerController=new ig.playerController();},changeState:function(newState){this.currentState=newState;switch(this.currentState){case this.STATE.PLAYMODE:this.sceneController.loadLevel(1);this.player=ig.game.getEntitiesByType(EntityQubber)[0];this.playerController.addPlayer(this.player);break;case this.STATE.MAINMENU:this.sceneController.loadLevel(0);break;}},loadLevel:function(data){this.parent(data);},update:function(){this.parent();switch(this.currentState){case this.STATE.PLAYMODE:this.playerController.updatePosition(this.player.pos.x,this.player.pos.y);this.screen.x=this.player.pos.x-ig.system.width/2;this.screen.y=this.player.pos.y-ig.system.height/2;if(ig.input.state('pause')){this.changeState(this.STATE.MAINMENU);}
...
break;default:break;break;}},draw:function(){this.parent();switch(this.currentState){case this.STATE.PLAYMODE:this.player.draw();this.font.draw('Q: '+this.player.money,this.drawCoordinates.score.x,this.drawCoordinates.score.y,ig.Font.ALIGN.LEFT);this.font.draw('Press J to pause',this.drawCoordinates.profile.x,this.drawCoordinates.profile.y,ig.Font.ALIGN.LEFT);break;case this.STATE.MAINMENU:this.biggerfont.draw(this.name,this.drawCoordinates.main.x,this.drawCoordinates.main.y,ig.Font.ALIGN.CENTER);this.bigfont.draw('Press Enter to start',this.drawCoordinates.main.x,this.drawCoordinates.main.y+50,ig.Font.ALIGN.CENTER);break;default:break;break;}}});ig.main('#canvas',MyGame,60,320,240,2);});